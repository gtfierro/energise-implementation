...
class Flexgrid_API(object):
    ...
    def __inv_select(self, inv_ids):
        ...
        for inv_id in inv_ids:
            ...
            print(inv_id)
            print(self.inverters[ind].inv_id)
    ...
    def enable_dyn_control(self, inv_ids):
        ...
        inverters = self.__inv_select(inv_ids)
        if self.parallel:
            self.__parallel_execution(Inverter.enable_dyn_control, inverters, debug = self.debug)
        else:
            self.__series_execution(Inverter.enable_dyn_control, inverters, debug = self.debug)
    ...
    def disable_dyn_control(self, inv_ids):
        ...
        inverters = self.__inv_select(inv_ids)
        if self.parallel:
            self.__parallel_execution(Inverter.disable_dyn_control, inverters, debug = self.debug)
        else:
            self.__series_execution(Inverter.disable_dyn_control, inverters, debug = self.debug)
...
### Battery Control
    ...
    def battery_max_export(self, P, inverters):
        ''' 
