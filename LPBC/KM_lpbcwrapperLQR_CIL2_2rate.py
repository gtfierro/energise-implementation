...
class lpbcwrapper(pbc.LPBCProcess): #this is related to super(), inherits attributes and behaviors from pbc.LPBCProcess (which is a wrapper for XBOSProcess)
    def __init__(self, cfg, busId, testcase, nphases, act_idxs, actType, plug_to_phase_idx, currentMeasExists, rate, numStepsPerActuation=1, localSratio=1, localVratio=1, ORT_max_kVA=500, VmagScaling=1):
        ...
        if self.controllerType == 'PI':
            ...
            '''
            # self.usingNonpuZeff should be 0 when using the 2 rate system (to make sure there is a controller when setting the disturbance)
    ...
    '''
    LPBC process has call_periodic built in, which runs the step function every self.rate seconds.
    ...
    def step(self, local_phasors, reference_phasors, phasor_target): #HERE what happens when no PMU readings are given (Gabe), maybe step wont be called
        ...
        if phasor_target is None and self.VangTarg_relative == 'initialize':
            ...
            if self.stepCount < self.numStepsPerActuation and self.controllerInitialized == 1:
                self.stepCount += 1
            ...
            elif self.pauseCount == 0: # self.pauseCount == 0 when a command hasnt been given yet
                ...
                #HHERE This may make passing the LQR P_implemented in LQRupdate, and even saving self.P_implemented in the warpper, redundant
                if self.controllerType == 'LQR':
                    self.controller.setu(self.P_implemented_PU, self.Q_implemented_PU) #record that a u command was implemented
...
rate = 1
numStepsPerActuation = 10 #this many steps are taken at 'rate' seconds before an actuation step occurs. This allows the disturbance observer to act faster than the controller
print('rate ', rate)
