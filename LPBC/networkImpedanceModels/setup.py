...
#kludgey way to print line impedances: (may want to move this elsewhere)
printLineImpedances = 1
...
#this is a bit confusing, has all of the measurements in bus classes, which are a part of the parent "feeder" class
...
# Buses, loads, lines, actuators, etc. are all stored as dictionaries within this object
    def __init__(self,modelpath,loadfolder,loadpath,actpath,timesteps,subkVbase_phg,subkVAbase):
        ...
        #HERE maybe want to model shunt caps as their impedance, would need to put it in pu (but DSS takes in shunt caps in terms of power as well)
...
class bus:
    def __init__(self,desig,subkVbase_phg,subkVAbase,timesteps):
        ...
        #HERE should change these so angle ares all base 0 and in radians
...
class line(connector):
    def __init__(self, desig, timesteps):
        ...
        self.to_node = None
        self.to_phases = list() #Keith: think these should always be equal to from
...
def ZtoY(Zmat):
    ...
    return np.linalg.pinv(tempZ) # HERE changed this
...
def propogatebasesdown(tree,currnode,kVbase_phg):
    # Sets the voltage bases of every node on a path downtream from a given node. Stops if it hits a transformer. #HERE why does it stop when it hits a transformer??
    # 'tree' should be a networkx graph object
    succlist = list()
    currnode.kVbase_phg = kVbase_phg
    currnode.Zbase = kVbase_phg*kVbase_phg*1000/currnode.kVAbase
    ...
    for iedge in edgelist:
        ...
        if isinstance(conn,transformer):
            if currnode == conn.w0_node:
                #propogatebasesup(tree,currnode,conn.w0_kVbase_phg) #HERE not sure what these are used for, and they create a bug when nodes 709 and 775 in 37 node feeder transformer model are in the original order
                propogatebasesdown(tree,iedge[1],conn.w1_kVbase_phg) #new base passed down from here
...
def Vsrcbuilder(modeldata, busdict):
    ...
    for idx, row in Vsrcsheet.iterrows():
        ...
        Vsrcdict[indkey].Vvec_phg[0,0] = Vsrcdict[indkey].kV_phph/np.sqrt(3) #HERE havent checked this but dont think its critical for calculating Y
...
# #Just a heads up:there is a more recent version of loadbuilder (and actbuilder) that includes timestepcur so that the SPBC can be started from step:
...
def transbuilder(modeldata,busdict,subkVAbase,timesteps):
    ...
    for idx in range(1,windcols+1): #HERE added these =1s after I upgraded pandas
        transsheet.iloc[0][idx] = 'w0_' + transsheet.iloc[0][idx]
    for idx in range(windcols+1,2*windcols+1): #and HERE
    ...
    for idx, row in transsheet.iterrows():
        if row['w0_bus a']:
            indkeyw0 = row['w0_bus a'][:len(row['w0_bus a'])-2] #this just gets the bus key wo the phase
        elif row['w0_bus b']:
        ...
        transdict[indkey].w0_kVbase_phg = row['w0_kV (ph-ph RMS)']/np.sqrt(3) #HERE checked this and its correct
...
def set_per_unit(linedict):
    ...
    for key, iconn in linedict.items():
        ...
        if printLineImpedances:
            print('*********************************************************************')
            print(f'line {key} Zbase: {iconn.Zbase}')
            print(f'line {key} Zpu: {iconn.Zpu}')
