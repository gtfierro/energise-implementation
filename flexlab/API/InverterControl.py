...
class ModbusRTUClient(ModbusSerialClient):
    def __init__(self, portName='com4', baudrate=115200):
        ModbusSerialClient.__init__(self,"rtu", port=portName, baudrate=baudrate, timeout=2)
...
class Inverter(object):
    ...
    def enable_dyn_power_control(self, debug=False):
        ...
        # Set fallback P
        self.client.write_registers(0xF312, float32_to_data_inv(7000), unit=self.inv_id)
        # Set Fallback CosPhi
        self.client.write_registers(0xF314, float32_to_data_inv(1), unit=self.inv_id)
        # Set P ramp up rate
        self.client.write_registers(0xF318, float32_to_data_inv(100), unit=self.inv_id)
        # Set P ramp down rate
        self.client.write_registers(0xF31A, float32_to_data_inv(100), unit=self.inv_id) 
        # Set Q ramp up rate
        self.client.write_registers(0xF31C, float32_to_data_inv(100), unit=self.inv_id)
        # Set Q ramp down rate
        self.client.write_registers(0xF31E, float32_to_data_inv(100), unit=self.inv_id)
        # Set CosPhi rate
        self.client.write_registers(0xF320, float32_to_data_inv(3.1415), unit=self.inv_id) # set to Pi
    ...
    def set_dyn_P(self, P, debug=False):
        ...
        if P < 0 or P > 100:
            print('error - out of range')
            return
        self.client.write_registers(0xF322, float32_to_data_inv(P), unit=self.inv_id)
    ...
    def set_dyn_Q(self, Q, safety = True, debug=False):
        ...
        self.client.write_registers(0xF324, float32_to_data_inv(Q), unit=self.inv_id)
